...
def select_relevant_tables(tables, columns_data, threshold=0.7):
...
def test_ssh_connection(host, port, user, private_key=None, password=None, timeout=10):
    """Teste la connectivité réseau et l'authentification SSH."""
    ...
    if private_key or password:
        try:
            ...
            if private_key and os.path.exists(private_key):
                ...
                ssh.connect(host, port=port, username=user, key_filename=private_key, timeout=timeout)
                logging.info(f"Authentification SSH réussie avec clé privée pour {host}:{port}")
            elif password:
                logging.info(f"Tentative d'authentification SSH avec mot de passe")
...
def discover_postgres(source, spark, source_index):
    ...
    # Supprimer le cache pour MAVIS pour forcer une nouvelle tentative
    if source['name'] == 'MAVIS' and os.path.exists(cache_path):
        logging.info(f"Suppression du cache pour {source['name']} afin de retenter la connexion")
    ...
    try:
        ...
        if ssh_cfg and ssh_cfg.get("host") and ssh_cfg.get("port") and ssh_cfg.get("user"):
            ...
            # Tenter l'authentification avec mot de passe en priorité (puisque la clé privée échoue)
            try:
                if ssh_password:
    ...
    with ThreadPoolExecutor(max_workers=4) as executor:
        ...
        for future in as_completed(future_to_source):
            ...
            try:
                ...
                if tables_info and isinstance(tables_info, list) and any("error" in info for info in tables_info):
                    logging.error(f"Échec de la découverte pour {source['name']}: {tables_info}")
                    print(f"❌ Échec de la découverte pour {source['name']} (postgres): {tables_info[0].get('error', 'Erreur inconnue')}")
                else:
    ...
    for source in sources:
        if source["type"] == "excel":
            try:
                ...
                if tables_info and isinstance(tables_info, list) and any("error" in info for info in tables_info):
                    logging.error(f"Échec de la découverte pour {source['name']}: {tables_info}")
                    print(f"❌ Échec de la découverte pour {source['name']} (excel): {tables_info[0].get('error', 'Erreur inconnue')}")
                else:
                    logging.info(f"Découverte réussie pour {source['name']}")
